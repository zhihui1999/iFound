...
public class GosControlModuleBaseActivity extends GosBaseActivity {
	...
	// 数据点"switch_gps"对应的标识名
	protected static final String KEY_SWITCH_GPS = "switch_gps";
	// 数据点"switch_bluetooth"对应的标识名
	protected static final String KEY_SWITCH_BLUETOOTH = "switch_bluetooth";
	// 数据点"alarm"对应的标识名
	protected static final String KEY_ALARM = "alarm";
	// 数据点"power"对应的标识名
	protected static final String KEY_POWER = "power";
	// 数据点"gps_signal"对应的标识名
	protected static final String KEY_GPS_SIGNAL = "gps_signal";
	// 数据点"latitude"对应的标识名
	protected static final String KEY_LATITUDE = "latitude";
	// 数据点"longitude"对应的标识名
	protected static final String KEY_LONGITUDE = "longitude";
	// 数据点"gpsTime"对应的标识名
	protected static final String KEY_GPSTIME = "gpsTime";
	// 数据点"beiJingTime"对应的标识名
	protected static final String KEY_BEIJINGTIME = "beiJingTime";
	...
	// 数据点"switch_gps"对应的存储数据
	protected static boolean data_switch_gps;
	// 数据点"switch_bluetooth"对应的存储数据
	protected static boolean data_switch_bluetooth;
	// 数据点"alarm"对应的存储数据
	protected static boolean data_alarm;
	// 数据点"power"对应的存储数据
	protected static int data_power;
	// 数据点"gps_signal"对应的存储数据
	protected static boolean data_gps_signal;
	// 数据点"latitude"对应的存储数据
	protected static byte[] data_latitude;
	// 数据点"longitude"对应的存储数据
	protected static byte[] data_longitude;
	// 数据点"gpsTime"对应的存储数据
	protected static byte[] data_gpsTime;
	// 数据点"beiJingTime"对应的存储数据
	protected static byte[] data_beiJingTime;
	...
	protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
		...
		if (dataMap.get("data") != null) {
			...
			for (String dataKey : map.keySet()) {
				...
				if (dataKey.equals(KEY_SWITCH_GPS)) {
					data_switch_gps = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_SWITCH_BLUETOOTH)) {
					data_switch_bluetooth = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_ALARM)) {
					data_alarm = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_POWER)) {
			
					data_power = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_GPS_SIGNAL)) {
			
					data_gps_signal = (Boolean) map.get(dataKey);
				}
				if (dataKey.equals(KEY_LATITUDE)) {
			
					data_latitude = (byte[]) map.get(dataKey);
				}
				if (dataKey.equals(KEY_LONGITUDE)) {
			
					data_longitude = (byte[]) map.get(dataKey);
				}
				if (dataKey.equals(KEY_GPSTIME)) {
			
					data_gpsTime = (byte[]) map.get(dataKey);
				}
				if (dataKey.equals(KEY_BEIJINGTIME)) {
			
					data_beiJingTime = (byte[]) map.get(dataKey);
				}
			}
		}

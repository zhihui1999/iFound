...
public class GosDeviceListAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		holder.getxiangqing().setOnClickListener(new OnClickListener() {

			@Override
			public void onClick(View v) {
				Message message = new Message();
				message.what = XIANGQING;
				message.obj = device.getDid().toString();
				handler.sendMessage(message);
			}
		});
...
class Holder {
	...
	private RelativeLayout delete2,xiangqing;
	...
	public RelativeLayout getxiangqing() {
		if (null == xiangqing) {
			xiangqing = (RelativeLayout) view.findViewById(R.id.xiangqing);
		}
		return xiangqing;
	}
